...
// NOTE: Your original file referenced these components in the Goal detail view
// but they were NOT imported in the snippet you pasted. Keeping the behavior
// would require these imports to exist in your project.
// If they already exist in your app, uncomment these:
// import GoalProgressChart from "@/components/goals/GoalProgressChart";
// import GoalUpdates from "@/components/goals/GoalUpdates";
...
export default function ClientDetail() {
  ...
  // Accordion / dropdown navigation
  const [openSection, setOpenSection] = useState("sessions");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
                {client.email && (
                  <div className="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    <a href={`mailto:${client.email}`} className="hover:text-slate-800">
                      {client.email}
                    </a>
                  </div>
                )}
                {client.phone && (
                  <div className="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    <a href={`tel:${client.phone}`} className="hover:text-slate-800">
                      {client.phone}
                    </a>
                  </div>
                )}
              ...
              {client.coaching_focus && (
                <div className="mt-4 p-3 bg-slate-50 rounded-lg">
                  <p className="text-sm text-slate-600">
                    <span className="font-medium text-slate-700">Coaching Focus:</span>{" "}
                    {client.coaching_focus}
                  </p>
                </div>
              )}
        ...
        {/* NEW: dropdown nav + accordion sections */}
        <div className="bg-white rounded-2xl border border-slate-100 p-4 sm:p-6">
          <div className="flex flex-col md:flex-row md:items-center gap-3">
            <div className="text-sm text-slate-500">Jump to:</div>
            <div className="w-full md:w-[360px]">
              <Select value={openSection} onValueChange={handleJump}>
          ...
          <Accordion
            ...
            collapsible
            value={openSection}
            onValueChange={(val) => setOpenSection(val || "sessions")}
            className="w-full"
          >
            {/* Sessions */}
            <AccordionItem
              value="sessions"
              ref={(node) => {
                if (node) sectionRefs.current.sessions = node;
              }}
            >
                ...
                <div className="space-y-4 pt-2">
                  ...
                  {sessions.length > 0 ? (
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  ...
                                  <DropdownMenuItem
                                    ...
                                    onClick={() =>
                                      setDeleteDialog({ open: true, type: "session", id: session.id })
                                    }
                                  >
                    ...
                    <div className="text-center py-12 bg-white rounded-xl border border-slate-100">
                      ...
                      <Button variant="link" onClick={() => setShowSessionForm(true)} className="mt-2">
            ...
            <AccordionItem
              value="goals"
              ref={(node) => {
                if (node) sectionRefs.current.goals = node;
              }}
            >
              <AccordionTrigger className="text-left">
                <span className="inline-flex items-center gap-2">
                  ...
                  {!selectedGoal ? (
                    <>
                      ...
                      {goals.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {goals.map((goal, index) => (
                            <div key={goal.id} className="relative group">
                              <GoalCard goal={goal} index={index} onClick={() => setSelectedGoal(goal)} />
                              <Button
                                variant="ghost"
                                size="icon"
                        ...
                        <div className="text-center py-12 bg-white rounded-xl border border-slate-100">
                          ...
                          <Button variant="link" onClick={() => setShowGoalForm(true)} className="mt-2">
                            Add the first goal
                          </Button>
                        </div>
                      )}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-white rounded-xl border border-slate-100 p-6">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Badge className="capitalize">
                                {String(selectedGoal.status || "").replace("_", " ")}
                              </Badge>
                            </div>
                          </div>
                          <Button
                        ...
                        {/* If you want the original nested Tabs for goal details, keep them here.
                           Your pasted snippet referenced GoalProgressChart/GoalUpdates, which were not imported.
                           Re-add them if those components exist in your project. */}
                        <div className="text-sm text-slate-500">
                          Goal detail view is unchanged — re-add your Progress Chart / Updates UI here if desired.
                        </div>
                      </div>
                    </div>
                  )}
            ...
            <AccordionItem
              value="messages"
              ref={(node) => {
                if (node) sectionRefs.current.messages = node;
              }}
            >
              <AccordionTrigger className="text-left">
                <span className="inline-flex items-center gap-2">
                  <MessageSquare className="w-4 h-4" />
                  Messages ({unreadClientMessagesCount})
                </span>
              </AccordionTrigger>
              <AccordionContent>
                <div className="space-y-4 pt-2">
                  <h3 className="font-semibold text-slate-800">Direct Messages</h3>
            ...
            <AccordionItem
              value="emails"
              ref={(node) => {
                if (node) sectionRefs.current.emails = node;
              }}
            >
              <AccordionTrigger className="text-left">
                <span className="inline-flex items-center gap-2">
            ...
            <AccordionItem
              value="payments"
              ref={(node) => {
                if (node) sectionRefs.current.payments = node;
              }}
            >
              <AccordionTrigger className="text-left">
                <span className="inline-flex items-center gap-2">
            ...
            <AccordionItem
              value="resources"
              ref={(node) => {
                if (node) sectionRefs.current.resources = node;
              }}
            >
              <AccordionTrigger className="text-left">
                <span className="inline-flex items-center gap-2">
                ...
                <div className="space-y-4 pt-2">
                  ...
                  {assignedResources.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {assignedResources.map((resource, index) => (
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            <div
                              className={`p-2 rounded-lg ${
                                resource.featured ? "bg-amber-50 text-amber-600" : "bg-slate-50 text-slate-600"
                              }`}
                            >
                          ...
                          <div className="flex items-center gap-2">
                            <Badge className="bg-violet-100 text-violet-700">{resource.category}</Badge>
                    ...
                    <div className="text-center py-12 bg-white rounded-xl border border-slate-100">
                      ...
                      <Button variant="link" onClick={() => setShowResourceDialog(true)}>
            ...
            <AccordionItem
              value="intake"
              ref={(node) => {
                if (node) sectionRefs.current.intake = node;
              }}
            >
              <AccordionTrigger className="text-left">
                <span className="inline-flex items-center gap-2">
              ...
              <AccordionContent>
                <div className="space-y-4 pt-2">
                  <h3 className="font-semibold text-slate-800">Intake Questionnaires</h3>
                        ...
                        >
                          <div className="flex items-start justify-between mb-4">
                            <div>
                              ...
                              <p className="text-sm text-slate-500">
                                Submitted{" "}
                                {format(
                                  new Date(questionnaire.completed_date || questionnaire.created_date),
                          ...
                          {questionnaire.responses && Object.keys(questionnaire.responses).length > 0 ? (
                            <div className="space-y-4 mt-6">
                              {Object.entries(questionnaire.responses).filter(([_, value]) => value && (!Array.isArray(value) || value.length > 0))
                                .length > 0 ? (
                                ...
                                <div className="text-center py-8 text-slate-500">
                                  ...
                                  <p className="text-xs mt-1">
                                    Please ask the client to resubmit or complete the questionnaire.
                                  </p>
                                </div>
                              )}
                            </div>
            ...
            <AccordionItem
              value="notes"
              ref={(node) => {
                if (node) sectionRefs.current.notes = node;
              }}
            >
              <AccordionTrigger className="text-left">
                <span className="inline-flex items-center gap-2">
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <AINotesAssistant
                    ...
                    onInsertSuggestion={(suggestion) => {
                      const newNote = notesDraft ? `${notesDraft}\n\n${suggestion}` : suggestion;
      ...
      {/* Email Composer */}
      <EmailComposer open={showEmailComposer} onClose={() => setShowEmailComposer(false)} client={client} />
